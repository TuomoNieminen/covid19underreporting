...
We utilised the No-U-Turn Sampler algorithm for sampling, which is an efficient Markov Chain Monte Carlo algorithm (\cite{homan2014}). We used STAN and the R package Rstan to carry out the sampling and monitored convergence via the Rhat statistic (\cite{carpenter2017}, \cite{R2021} ,\cite{standev2020rstan}). The STAN code is given in the Supplement: STAN code.
...
The STAN code which implements the statistical methods described, and an R code example which demonstrates how to apply the methods to the data described in the article, is available in \href{}{GitHub}.
...
\section{Supplement: STAN code}

\lstinputlisting[]{code/model.stan}
